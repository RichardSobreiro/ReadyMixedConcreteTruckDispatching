...
def googleMapsCombinations(mixerTrucks, loadingPlaces, deliveries, orders, 
    ...
    nK = len(mixerTrucks)
    nN = len(loadingPlaces) + nJ
    nA = len(loadingPlaces) + nJ
    nL = 8
    revenues = np.zeros((nJ))
    codLoadingPlants = np.zeros((len(loadingPlaces)))
    codMixerTrucks = np.zeros((len(mixerTrucks)))
    codOrders = np.zeros((nJ))
    codDeliveries = np.zeros((nJ))
    a = np.zeros((nJ))
    b = np.zeros((nJ))
    c = np.zeros((len(loadingPlaces), nJ))
    t = np.zeros((len(loadingPlaces), nJ))
    csd = np.zeros((nJ))
    ...
    datfile = open(basePath + '\\RMCTDP_Combinations.dat', 'w+')
